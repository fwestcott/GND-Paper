...
All_data_split_df <- mutate(All_data, "Liver_fat" = if_else(All_data$`IHTG (%)`<=5.56, "Low","High"), "Insulin" = if_else(All_data$`Insulin (nmol/L)`<=11.2, "Low","High"))

```
``` {r}

Liver_fat_counts <- All_data_split_df %>% 
  group_by(`Liver_fat`) %>% count(Sex)
...
Keepers <- c("Liver_fat", "Insulin", row.names(Liver_fat_medians))
...
Results <- as.data.frame(Keepers, row.names = Keepers)


for (i in 3:ncol(All_data_split_filtered)) {
   formula <- as.formula(paste("`", colnames(All_data_split_filtered)[i], "`", " ~ Liver_fat", sep=""))
   model <- wilcox_test(data = All_data_split_filtered, formula, exact = FALSE) %>% add_significance()

   Results[i,1] <- paste(model$p.adj.signif)
...
for (i in 3:ncol(All_data_split_filtered)) {
   ...
   Results[i,2] <- paste(model$p.adj.signif)

}


Results <- Results[3:24,] %>% rename("Liver fat P-value" = Keepers, "Insulin P-value" = V2)


```

``` {R, Tablulate!}
Liver_fat_table <- cbind(Liver_fat_medians, Liver_fat_mins, Liver_fat_maxs, Results)

Liver_fat_table <- mutate(Liver_fat_table, "Low (<= 5.56%)", "High (> 5.56%)")

for(i in 1:nrow(Liver_fat_table)) {
  Liver_fat_table[i,9] <- paste(Liver_fat_table[i,2], "(", Liver_fat_table[i,4], "-", Liver_fat_table[i,6], ")", sep = " ")
  Liver_fat_table[i,10] <- paste(Liver_fat_table[i,1], "(", Liver_fat_table[i,3], "-", Liver_fat_table[i,5], ")", Liver_fat_table[i,7], sep = " ")
}

Liver_fat_table <- Liver_fat_table[,9:10]
...
Insulin_table <- cbind(Insulin_medians, Insulin_mins, Insulin_maxs, Results)

Insulin_table <- mutate(Insulin_table, "Low (<=11.2nmol/L)", "High (>11.2nmol/L)")

for(i in 1:nrow(Insulin_table)) {
  Insulin_table[i,9] <- paste(Insulin_table[i,2], "(", Insulin_table[i,4], "-", Insulin_table[i,6], ")", sep = " ")
  Insulin_table[i,10] <- paste(Insulin_table[i,1], "(", Insulin_table[i,3], "-", Insulin_table[i,5], ")", Insulin_table[i,8], sep = " ")
}

Insulin_table <- Insulin_table[,9:10]
